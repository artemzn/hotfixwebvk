...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  ...
  let fasterTmp = true
  let timeTmp = ''
  let selfServiceTmp = false
  if (localStorage.getItem("ordersSettings")) {
    let item = JSON.parse(localStorage.getItem("ordersSettings"));
    fasterTmp = item.faster;
    timeTmp = item.time;
    selfServiceTmp = item.selfService;
  }
  const [ faster, setFaster ] = useState(fasterTmp);
  const [ time, setTime ] = useState(timeTmp);
  const [ selfService, setSelfService ] = useState(selfServiceTmp);
  ...
  useEffect(() => {
    localStorage.setItem('ordersSettings', JSON.stringify({
      faster,
      time,
      selfService
    }));
